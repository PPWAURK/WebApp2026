...
const TRAINING_QUIZ_LINK_LANGUAGES = ['fr', 'bn'];
function isTrainingQuizLinkLanguage(value) {
    return TRAINING_QUIZ_LINK_LANGUAGES.includes(value);
}
let UsersService = class UsersService {
    ...
    async listTrainingQuizLinks() {
        ...
        const savedLinkMap = new Map(savedLinks.map((entry) => [
            `${entry.section}:${entry.language.toLowerCase()}`,
            entry.quizUrl,
        ]));
        return this.getAllTrainingSections().flatMap((section) => TRAINING_QUIZ_LINK_LANGUAGES.map((language) => ({
            section,
            language,
            quizUrl: savedLinkMap.get(`${section}:${language}`) ?? null,
        })));
    }
    async upsertTrainingQuizLink(sectionRaw, languageRaw, quizUrlRaw, actorRole) {
        if (actorRole !== client_1.Role.ADMIN) {
            throw new common_1.BadRequestException('Only ADMIN can manage quiz links');
        }
        if (!(0, upload_taxonomy_1.isUploadSection)(sectionRaw)) {
            throw new common_1.BadRequestException('Invalid training section');
        }
        const language = languageRaw.toLowerCase();
        if (!isTrainingQuizLinkLanguage(language)) {
            throw new common_1.BadRequestException('Invalid quiz link language');
        }
    ...
    async updateEmployeeWorkplaceRole(userId, workplaceRole, actor) {
        this.ensureRoleScope(actor);
        const user = await this.prisma.user.findUnique({
            where: { id: userId },
            select: {
        ...
        if (user.role !== client_1.Role.EMPLOYEE) {
            throw new common_1.BadRequestException('Only EMPLOYEE workplace role can be updated');
        }
        if (actor.actorRole === client_1.Role.MANAGER &&
            user.restaurantId !== actor.actorRestaurantId) {
            throw new common_1.BadRequestException('Manager can only update users in own restaurant');
    ...
    normalizeQuizUrl(value) {
        ...
        try {
            ...
            throw new common_1.BadRequestException('quizUrl must be a valid URL');
        }
    }
};
